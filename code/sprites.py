...
class BG(pygame.sprite.Sprite):
    ...
    def update(self, dt):  # Тут будет движение фона с постоянной скоростью
        # а также возврат в начальное положение при достижении края
        self.pos.x -= 300 * dt
...
class Ground(pygame.sprite.Sprite):  # (Почти как с BG) Загрузка и масштабирование земли
    ...
    def update(self, dt):  # Движение земли с постоянной скоростью
        # Возврат в начальное положение при достижении края
        self.pos.x -= 360 * dt
        if self.rect.centerx <= 0:
            self.pos.x = 0
...
class Plane(pygame.sprite.Sprite):
    def __init__(self, groups, scale_factor):
        super().__init__(groups)  # Загрузка анимационных кадров
...
class Obstacle(pygame.sprite.Sprite):
    def __init__(self, groups, scale_factor):   # случайный выбор типа препятствия (верхнее/нижнее)
                                                # загрузка и масштабирование изображения
                                                # Случайное позиционирование за пределами экрана
